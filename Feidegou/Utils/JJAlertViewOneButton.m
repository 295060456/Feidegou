...
-(void)showAlertView:(UIViewController *)controller andTitle:(NSString *)title andMessage:(NSString *)message andCancel:(NSString *)cancel andCanelIsRed:(BOOL)isRed  andBack:(void (^)())cancle{
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0) {
        ...
        if (cancel) {
            ...
            UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:cancel style:style handler:^(UIAlertAction *action) {
                cancle();
            }];
        ...
        UIAlertView *TitleAlert = [[UIAlertView alloc] initWithTitle:title message:message delegate:self cancelButtonTitle:cancel otherButtonTitles:nil,nil];
        [TitleAlert show];
    }
}
