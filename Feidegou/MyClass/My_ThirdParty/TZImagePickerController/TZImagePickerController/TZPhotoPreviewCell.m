            ...
            [[TZImageManager manager] getOriginalPhotoDataWithAsset:model.asset
                ...
                if (!isDegraded) {
                    ...
                    if ([TZImagePickerConfig sharedInstance].gifImagePlayBlock) {
                        [TZImagePickerConfig sharedInstance].gifImagePlayBlock(self,
                                                                               self.imageView,
                                                                               data,
                                                                               info);
                    } else {
...
- (void)setAsset:(PHAsset *)asset {
    ...
    self.imageRequestID = [[TZImageManager manager] getPhotoWithAsset:asset
        ...
        if (self.imageView.tz_height && self.allowCrop) {
            CGFloat scale = MAX(self.cropRect.size.width / self.imageView.tz_width,
                                self.cropRect.size.height / self.imageView.tz_height);
...
- (void)resizeSubviews {
    ...
    if (_imageContainerView.tz_height > self.tz_height &&
        _imageContainerView.tz_height - self.tz_height <= 1) {
...
- (void)refreshScrollViewContentSize {
    if (_allowCrop) {
        ...
        if (contentHeightAdd > 0 || contentWidthAdd > 0) {
            _scrollView.contentInset = UIEdgeInsetsMake(contentHeightAdd,
                                                        _cropRect.origin.x,
                                                        0,
                                                        0);
        } else {
