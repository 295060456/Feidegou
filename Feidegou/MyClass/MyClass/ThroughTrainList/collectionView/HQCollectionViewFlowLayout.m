...
- (NSArray *) layoutAttributesForElementsInRect:(CGRect)rect{
    ...
    @weakify(self)
    [noneHeaderSections enumerateIndexesUsingBlock:^(NSUInteger idx,
                                                     BOOL *stop){
        @strongify(self)
    ...
    for (UICollectionViewLayoutAttributes *attributes in superArray) {
        ...
        if ([attributes.representedElementKind isEqualToString:UICollectionElementKindSectionHeader]){
            ...
            //得到第一个item的indexPath
            NSIndexPath *firstItemIndexPath = [NSIndexPath indexPathForItem:0
                                                                  inSection:attributes.indexPath.section];
            //得到最后一个item的indexPath
            NSIndexPath *lastItemIndexPath = [NSIndexPath indexPathForItem:MAX(0, numberOfItemsInSection - 1)
                                                                 inSection:attributes.indexPath.section];
    ...
    }return [superArray copy];//转换回不可变数组，并返回
}
