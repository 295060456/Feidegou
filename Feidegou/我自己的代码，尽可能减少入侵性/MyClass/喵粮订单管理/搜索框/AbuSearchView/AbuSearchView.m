...
@property(nonatomic,strong)NSMutableArray *stcokArray;//股票数据
@property(nonatomic,retain)NSMutableArray *resultArr;//匹配结果数据
...
-(void)ajustIconWith:(AbuSearchBarIconAlign)iconAlign{
    if (_iconAlign == SearchBarIconAlignCenter) { //是否居中
        ...
        self.textField.frame = CGRectMake(7,
                                          7,
        ...
        if (x > 0) {
            _iconCenterView.frame = CGRectMake(x,
                                               0,
                                               _iconCenterView.frame.size.width,
                                               _iconCenterView.frame.size.height);
...
- (BOOL)textFieldShouldEndEditing:(UITextField *)textField{
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(searchBarShouldEndEditing:)]){
...
//股票搜索全匹配比较函数
- (BOOL) searchEqualToString:(NSString*)text
                      string:(NSString*)searchString {
...
replacementString:(NSString *)string{
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(searchBar:shouldChangeTextInRange:replacementText:)]){
        return [self.delegate
                searchBar:self
                shouldChangeTextInRange:range replacementText:string];
    }return YES;
}
/**
 * 清空textField的值
...
- (BOOL)textFieldShouldClear:(UITextField *)textField{
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(searchBar:textDidChange:)]){
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField{
    [_textField resignFirstResponder];
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(searchBarSearchButtonClicked:)]){
...
- (UIButton *)cancelButton{
    if (!_cancelButton) {
        ...
        _cancelButton.frame = CGRectMake(self.frame.size.width - 60,
                                         7,
                                         60,
                                         30);
...
- (UITextField *)textField{
    if (!_textField) {
        _textField = [[UITextField alloc] initWithFrame:CGRectMake(7,
                                                                   7,
                                                                   self.frame.size.width - 7 * 2,
                                                                   30)];
