...
@property (nonatomic, assign) CGFloat photoWidth;/// The pixel width of output image, Default is 828px / 导出图片的宽度，默认828像素宽
@property (nonatomic, assign) NSInteger columnNumber;/// Default is 4, Use in photos collectionView in TZPhotoPickerController 默认4列, TZPhotoPickerController中的照片collectionView
...
/// Get photo 获得照片
- (PHImageRequestID)getPostImageWithAlbumModel:(TZAlbumModel *)model
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset
                           completion:(void (^)(UIImage *photo,
                                                NSDictionary *info,
                                                BOOL isDegraded))completion
                      progressHandler:(void (^)(double progress,
                                                NSError *error,
                                                BOOL *stop,
...
- (PHImageRequestID)getOriginalPhotoWithAsset:(PHAsset *)asset
                                   completion:(void (^)(UIImage *photo,
                                                        NSDictionary *info))completion;
- (PHImageRequestID)getOriginalPhotoWithAsset:(PHAsset *)asset
                                newCompletion:(void (^)(UIImage *photo,
                                                        NSDictionary *info,
                                                        BOOL isDegraded))completion;
- (PHImageRequestID)getOriginalPhotoWithAsset:(PHAsset *)asset
                              progressHandler:(void (^)(double progress,
...
- (void)getVideoOutputPathWithAsset:(PHAsset *)asset
                         completion:(void (^)(NSString *outputPath))completion __attribute__((deprecated("Use -getVideoOutputPathWithAsset:failure:success:")));
