...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = [UIColor colorWithRed:(83/255.0)
                                                      green:(179/255.0)
                                                       blue:(17/255.0)
                                                      alpha:1.0];
    self.oKButtonTitleColorDisabled = [UIColor colorWithRed:(83/255.0)
                                                      green:(179/255.0)
                                                       blue:(17/255.0)
                                                      alpha:0.5];
    
    self.navigationBar.barTintColor = [UIColor colorWithRed:(34/255.0)
                                                      green:(34/255.0)
                                                       blue:(34/255.0)
                                                      alpha:1.0];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount
    ...
    if (self) {
        ...
        if (![[TZImageManager manager] authorizationStatusAuthorized]) {
            ...
            _tipLabel.frame = CGRectMake(8,
                                         120,
                                         self.view.tz_width - 16,
                                         60);
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets
    ...
    if (self) {
        ...
        @weakify(self)
        [previewVc setDoneButtonClickBlockWithPreviewType:^(NSArray<UIImage *> *photos,
                                                            NSArray *assets,
...
- (void)configDefaultSetting {
    ...
    self.iconThemeColor = [UIColor colorWithRed:31 / 255.0
                                          green:185 / 255.0
                                           blue:34 / 255.0
                                          alpha:1.0];
    ...
    self.cropRect = CGRectMake((self.view.tz_width - cropViewWH) / 2,
                               (self.view.tz_height - cropViewWH) / 2,
                               cropViewWH,
                               cropViewWH);
}
...
- (void)configDefaultImageName {
    ...
    self.photoNumberIconImage = [self createImageWithColor:nil
                                                      size:CGSizeMake(24, 24)
                                                    radius:12]; // @"photo_number_icon";
...
- (void)setShowSelectedIndex:(BOOL)showSelectedIndex {
    ...
    if (showSelectedIndex) {
        self.photoSelImage = [self createImageWithColor:nil
                                                   size:CGSizeMake(24, 24)
                                                 radius:12];
    }
...
- (UIAlertController *)showAlertWithTitle:(NSString *)title {
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:[NSBundle tz_localizedStringForKey:@"OK"]
...
- (void)setCircleCropRadius:(NSInteger)circleCropRadius {
    ...
    self.cropRect = CGRectMake(self.view.tz_width / 2 - circleCropRadius,
...
- (void)setCropRect:(CGRect)cropRect {
    ...
    _cropRectLandscape = CGRectMake((self.view.tz_height - widthHeight) / 2,
                                    cropRect.origin.x,
                                    widthHeight,
                                    widthHeight);
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (PHAsset *asset in selectedAssets) {
        TZAssetModel *model = [TZAssetModel modelWithAsset:asset
                                                      type:[[TZImageManager manager] getAssetType:asset]];
...
- (void)viewDidLayoutSubviews {
    ...
    _progressHUD.frame = CGRectMake(0,
                                    progressHUDY,
                                    self.view.tz_width,
                                    self.view.tz_height - progressHUDY);
    _HUDContainer.frame = CGRectMake((self.view.tz_width - 120) / 2,
                                     (_progressHUD.tz_height - 90 - progressHUDY) / 2,
                                     120,
                                     90);
    _HUDIndicatorView.frame = CGRectMake(45,
                                         15,
                                         30,
                                         30);
    _HUDLabel.frame = CGRectMake(0,
                                 40,
                                 120,
                                 50);
}
...
- (void)configTableView {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo
                             allowPickingImage:imagePickerVc.allowPickingImage
                               needFetchAssets:!self_weak_.isFirstAppear
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (self_weak_.isFirstAppear) {
                    self_weak_.isFirstAppear = NO;
                    [self_weak_ configTableView];
                }
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    _languageBundle = [NSBundle bundleWithPath:[[NSBundle tz_imagePickerBundle] pathForResource:preferredLanguage
                                                                                         ofType:@"lproj"]];
}
