...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(reload:)
                                                 name:@"TZ_PHOTO_PICKER_RELOAD_NOTIFICATION"
                                               object:nil];
    return self;
}
...
- (void)setModel:(TZAssetModel *)model {
    ...
    if (imageRequestID &&
        self.imageRequestID &&
        imageRequestID != self.imageRequestID) {
        [[PHImageManager defaultManager] cancelImageRequest:self.imageRequestID];
    ...
    if (self.assetCellDidSetModelBlock) {
        self.assetCellDidSetModelBlock(self,
                                       _imageView,
                                       _selectImageView,
                                       _indexLabel,
                                       _bottomView,
...
- (void)setType:(TZAssetCellType)type {
    _type = type;
    if (type == TZAssetCellTypePhoto ||
        type == TZAssetCellTypeLivePhoto ||
        (type == TZAssetCellTypePhotoGif && !self.allowPickingGif) ||
...
- (void)reload:(NSNotification *)noti {
    ...
    if (tzImagePickerVc.selectedModels.count >= tzImagePickerVc.maxImagesCount &&
        tzImagePickerVc.showPhotoCannotSelectLayer &&
...
- (UIButton *)selectPhotoButton {
    if (!_selectPhotoButton) {
        ...
        [selectPhotoButton addTarget:self
                              action:@selector(selectPhotoButtonClick:)
...
- (UIView *)bottomView {
    if (!_bottomView) {
        ...
        bottomView.backgroundColor = [UIColor colorWithRed:rgb
                                                     green:rgb
                                                      blue:rgb
                                                     alpha:0.8];
...
- (void)layoutSubviews {
    ...
    if (self.allowPreview) {
        _selectPhotoButton.frame = CGRectMake(self.tz_width - 44,
                                              0,
                                              44,
                                              44);
    } else {
    ...
    _selectImageView.frame = CGRectMake(self.tz_width - 27,
                                        3,
                                        24,
                                        24);
    ...
    _progressView.frame = CGRectMake(progressXY,
                                     progressXY,
                                     progressWH,
                                     progressWH);
    ...
    _timeLength.frame = CGRectMake(self.videoImgView.tz_right,
                                   0,
                                   self.tz_width - self.videoImgView.tz_right - 5,
                                   17);
    ...
    if (self.assetCellDidLayoutSubviewsBlock) {
        self.assetCellDidLayoutSubviewsBlock(self,
                                             _imageView,
                                             _selectImageView,
                                             _indexLabel,
                                             _bottomView,
...
- (void)setModel:(TZAlbumModel *)model {
    _model = model;
    NSMutableAttributedString *nameString = [[NSMutableAttributedString alloc] initWithString:model.name
                                                                                   attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16],
                                                                                                NSForegroundColorAttributeName:[UIColor blackColor]}];
    NSAttributedString *countString = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"  (%zd)",model.count]
                                                                      attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16],
    ...
    if (model.selectedCount) {
        ...
        [self.selectedCountButton setTitle:[NSString stringWithFormat:@"%zd",model.selectedCount]
...
- (void)layoutSubviews {
    [super layoutSubviews];
    _selectedCountButton.frame = CGRectMake(self.contentView.tz_width - 24,
                                            23,
                                            24,
                                            24);
    NSInteger titleHeight = ceil(self.titleLabel.font.lineHeight);
    self.titleLabel.frame = CGRectMake(80,
                                       (self.tz_height - titleHeight) / 2,
                                       self.tz_width - 80 - 50,
                                       titleHeight);
    self.posterImageView.frame = CGRectMake(0,
                                            0,
                                            70,
                                            70);
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _imageView.backgroundColor = [UIColor colorWithWhite:1.000
                                                       alpha:0.500];
