...
@interface GoodMainController ()<RefreshControlDelegate,DidClickDelegeteCollectionViewMainType,DidClickDelegeteOnlyCollectionView,SuspensionViewDelegate>
...
@property (nonatomic, strong) SuspensionView *btnSuspension;
@end
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(self.tabGood.contentOffset.y<-20) {
        ...
        self.viHeader.backgroundColor=ColorFromRGBA(252, 102, 159, self.tabGood.contentOffset.y /height);
    }
}
...
//        if ([TransformString(self.arrFloorList[indexPath.row][@"type"]) isEqualToString:@"photo"]) {
//            NSArray *array = self.arrFloorList[indexPath.row][@"modeList"];
//            if (array.count>0&&[array isKindOfClass:[NSArray class]]) {
//                NSInteger intHeight = [array[0][@"height"] integerValue];
//                NSInteger intWidth = [array[0][@"width"] integerValue];
//                if (intWidth>0&&intHeight>0) {
//                    return SCREEN_WIDTH*intHeight/intWidth;
//                }
//            }
//        }
        return [LocationHeaper heightByInfo:self.arrFloorList[indexPath.row]];
    }
...
//    <option value="10">领取奖励</option>
    ...
    if (intType == 1) {
        ...
        if ([[PersonalInfo sharedInstance] isShow]) {
            if ([[PersonalInfo sharedInstance] isLogined]) {
                UIStoryboard *storyboard = [UIStoryboard storyboardWithName:StoryboardPersonCenter bundle:nil];
                MenberMainController *controller = [storyboard instantiateViewControllerWithIdentifier:@"MenberMainController"];
                [self.navigationController pushViewController:controller animated:YES];
            }else{
        ...
        [alertView showAlertView:self andTitle:@"提示" andMessage:@"请更新到最新版本" andCancel:@"确定" andCanelIsRed:YES :^{
...
- (void)repeatAnimation:(NSTimer *)timer{
    if (self.arrTip.count>0) {
        ...
        CGFloat fWidth = [LocationHeaper conculuteRightCGSizeOfString:strMsg andWidth:SCREEN_WIDTH-10 andFont:12.0].width+15;
