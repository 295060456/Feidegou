...
@interface OrderTuihuoController ()<UIPickerViewDataSource,UIPickerViewDelegate>
...
- (void)initPinckerView{
    ...
    UIButton *btnDelete = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, CGRectGetHeight(self.viBack.frame))];
    [btnDelete handleControlEvent:UIControlEventTouchUpInside withBlock:^{
        [self hideSelete];
    }];
    [self.viBack addSubview:btnDelete];
    self.pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.viBack.frame)-240, SCREEN_WIDTH, 240)];
    [self.pickerView setBackgroundColor:[UIColor whiteColor]];
    [self.pickerView setDelegate:self];
    [self.pickerView setDataSource:self];
    [self.viBack addSubview:self.pickerView];
    UIView *viButton = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMinY(self.pickerView.frame)-40, SCREEN_WIDTH, 40)];
    [viButton setBackgroundColor:[UIColor whiteColor]];
    UIView *viLine = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(viButton.frame)-1, SCREEN_WIDTH, 1)];
    [viLine setBackgroundColor:ColorLine];
    UIButton *btnCancel = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 60, CGRectGetHeight(viButton.frame))];
    ...
    UIButton *btnConfilm = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-60, 0, 60, CGRectGetHeight(viButton.frame))];
...
- (IBAction)clickButtonConfilm:(UIButton *)sender {
    ...
    __weak OrderTuihuoController *myself = self;
    self.disposable = [[[JJHttpClient new] requestFourZeroWuliuRefund_id:[NSString stringStandard:self.modelOrderDetail.refund[@"refund_id"]] andcompany_id:[NSString stringStandard:self.arrData[self.row][@"id"]] andship_code:self.txtNum.text] subscribeNext:^(NSDictionary*dictionary) {
        [SVProgressHUD dismiss];
        if ([dictionary[@"code"] intValue]==1) {
            JJAlertViewOneButton *alertView = [[JJAlertViewOneButton alloc] init];
            [alertView showAlertView:self andTitle:nil andMessage:StringFormat(@"%@",dictionary[@"msg"])  andCancel:@"确定" andCanelIsRed:YES andBack:^{
                D_NSLog(@"点击了确定");
